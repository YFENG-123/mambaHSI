...
class Net1(nn.Module):
    def __init__(
        ...
        光谱特征提取层
        """
        # 1x1卷积：将bands维度投影到d_model维度
        self.conv1x1 = nn.Sequential(
            nn.Conv2d(bands, self.d_model, kernel_size=1, stride=1, padding=0),
    ...
    def forward(self, x):
        ...
        x_conv_spectrum = self.conv1x1(x_conv_spectrum)  # (1, d_model, H, W)
        ...
        x_conv_spatial = self.conv3x3(x_conv_spatial)  # (1, d_model, H, W)
        # 3x3卷积：进一步空间特征提取
        x_conv_spatial = self.conv3x3_2(x_conv_spatial)  # (1, d_model, H, W)
        ...
        x_flat = x_concat.reshape(-1, self.d_model * 2)  # (H*W, d_model * 2)
        x_seq = x_flat.unsqueeze(0)  # (1, H*W, d_model * 2)
        # Mamba层
        x_mamba = self.mamba(x_seq)  # (1, H*W, d_model * 2)
        x_mamba_flat = x_mamba.squeeze(0)  # (H*W, d_model * 2)

        # 分类
        output_flat = self.classifier(x_mamba_flat)  # (H*W, num_classes)
